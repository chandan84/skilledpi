...
from chibu.db.models import (  # noqa: F401 — all must be imported for Base.metadata
    Agent,
    AgentEvent,
    AgentGroup,
    AgentSession,
...
async def test_create_agent_assigns_unique_token(session, tmp_path):
    ...
    agent = await reg.create_agent("pi", "badmono org", str(tmp_path))
...
async def test_create_agent_creates_group_automatically(session, tmp_path):
    reg = AgentRegistry(session)
    agent = await reg.create_agent("pi", "new group", str(tmp_path))
...
async def test_two_agents_in_same_group_share_group_record(session, tmp_path):
    reg = AgentRegistry(session)
    a1 = await reg.create_agent("agent-a", "shared-group", str(tmp_path / "a"))
    a2 = await reg.create_agent("agent-b", "shared-group", str(tmp_path / "b"))
...
async def test_delete_agent(session, tmp_path):
    ...
    found = await reg.get_agent(agent.agent_id)
